...
const userData = new _data.UserData(_cookie.cookies.get(_cookie.cookies.names.userName), _cookie.cookies.get(_cookie.cookies.names.email));
if (_cookie.cookies.get(_cookie.cookies.names.email)) _network.network.socketInit(addMessageHundler);
function addMessageHundler(event) {
    const { user , text , createdAt  } = JSON.parse(event.data);
    const incomingUserData = new _data.UserData(user.name, user.email, text, _dateFns.format(new Date(createdAt), 'H:m'), true);
    if (userData.email === user.email) _view.renderMessage(userData);
    else _view.renderMessage(incomingUserData);
...
_view.UI.FORM_MESSAGE.FORM.addEventListener('submit', function(event) {
    event.preventDefault();
    formMessageHundler(_view.UI.FORM_MESSAGE.TEXTAREA.value);
    this.reset();
});
_view.UI.FORM_AUTH.addEventListener('submit', formAuthHundler);
_view.UI.FORM_CONFIR.addEventListener('submit', formConfirHundler);
_view.UI.FORM_SETTINGS.addEventListener('submit', formSettingsHundler);
...
function formAuthHundler(event) {
    event.preventDefault();
    if (_cookie.cookies.get(_cookie.cookies.names.token)) {
        alert(_error.ERROR_MESSAGES.TOKEN);
        return _popup.popupClose();
    }
    const email = new FormData(this).get('email');
    if (email === '') return alert(_error.ERROR_MESSAGES.EMPTY_STRING);
    if (!_error.isValidEmail(email)) return alert(_error.ERROR_MESSAGES.EMAIL_VALID);
    _network.network.authorizationRequest(email);
    _cookie.cookies.set(_cookie.cookies.names.email, email);
    _popup.popupOpen(_popup.POPUP_NAMES.CONFIR);
    this.reset();
}
function formConfirHundler(event) {
    event.preventDefault();
    const token = new FormData(this).get('token');
    if (token === '') return alert(_error.ERROR_MESSAGES.EMPTY_STRING);
    _cookie.cookies.set(_cookie.cookies.names.token, token);
    _network.network.socketInit(addMessageHundler);
    _view.renderMessages();
    _popup.popupOpen(_popup.POPUP_NAMES.SETTINGS);
    this.reset();
}
async function formSettingsHundler(event) {
    event.preventDefault();
    const userName = new FormData(this).get('username');
    if (userName === '') return alert(_error.ERROR_MESSAGES.EMPTY_STRING);
    if (!_cookie.cookies.get(_cookie.cookies.names.token)) return _popup.popupOpen(_popup.POPUP_NAMES.AUTH);
    _network.network.settingsRequest(userName, _cookie.cookies.get(_cookie.cookies.names.token));
    _cookie.cookies.set(_cookie.cookies.names.userName, userName);
    userData.userName = userName;
    _popup.popupClose();
    this.reset();
}

},{"../scss/main.scss":"kOw5V","./network":"8wofE","./cookie":"lfQWm","./data":"4vWRQ","./view":"kALtk","./popup":"jUySk","./error":"iq6dU","date-fns":"5VaJV"}],"kOw5V":[function() {},{}],"8wofE":[function(require,module,exports) {
...
class Network {
    ...
    async sendRequest(url, method, { body , token  } = {
        ...
        if (method === 'PATCH' || method === 'GET') options.headers.Authorization = `Bearer ${token}`;
        try {
            const response = await fetch(url, options);
            if (!response.ok) throw new _error.RequestError(`${_error.ERROR_MESSAGES.REQUEST_FAIL}: статус: ${response.status}`);
    ...
    async authorizationRequest(email) {
        return await this.sendRequest(this.endpoints.user, 'POST', {
            body: {
                email
            }
        });
    ...
    async settingsRequest(userName, token) {
        return await this.sendRequest(this.endpoints.user, 'PATCH', {
            body: {
                name: userName
            },
            token
    ...
    async getUserData(token) {
        return await this.sendRequest(this.endpoints.me, 'GET', {
            token
        });
    }
    async getHistoryMessages(token) {
        return await this.sendRequest(this.endpoints.messages, 'GET', {
            token
        });
    }
...
class Cookie {
    ...
    set(cookieName, value) {
        _jsCookieDefault.default.set(cookieName, value);
    }
    get(cookieName) {
        return _jsCookieDefault.default.get(cookieName);
    }
}
const cookies = new Cookie();

},{"js-cookie":"dxlEp","@parcel/transformer-js/src/esmodule-helpers.js":"j7FRh"}],"dxlEp":[function(require,module,exports) {
...
var _dateFns = require("date-fns");
function UserData(userName, email, message, date = _dateFns.format(new Date(), 'H:m'), incoming = false) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"j7FRh"}],"kALtk":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
async function renderMessages() {
    const email = _cookie.cookies.get(_cookie.cookies.names.email);
    if (!email) return;
...
function renderMessage({ userName , message , date , incoming  }) {
    ...
    if (incoming) messageArea.classList.add('message_incoming');
...
},{"./network":"8wofE","./data":"4vWRQ","date-fns":"5VaJV","./cookie":"lfQWm","@parcel/transformer-js/src/esmodule-helpers.js":"j7FRh"}],"jUySk":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "POPUP_NAMES", ()=>POPUP_NAMES
);
parcelHelpers.export(exports, "popupOpen", ()=>popupOpen
);
parcelHelpers.export(exports, "popupClose", ()=>popupClose
);
var _view = require("./view");
const POPUP_NAMES = {
    AUTH: 'authorization',
...
},{"./view":"kALtk","@parcel/transformer-js/src/esmodule-helpers.js":"j7FRh"}]},["kzhsr","aKvBm"], "aKvBm", "parcelRequire6143")
